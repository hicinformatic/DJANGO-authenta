...
urlpatterns.append(url(r'^authenta/task/details/(?P<pk>\d+).?(?P<extension>(html|json|txt))?$', views.TaskDetail.as_view(), name='TaskDetail'))
urlpatterns.append(url(r'^authenta/task/create.?(?P<extension>(html|json|txt))?/?$', views.TaskCreate.as_view(), name='TaskCreate'))
urlpatterns.append(url(r'^authenta/task/update.?(?P<extension>(html|json|txt))?/?$', views.TaskUpdate.as_view(), name='TaskUpdate'))
#urlpatterns.append(url(r'^authenta/task/create/(?P<tasktype>%s).?(?P<extension>(html|json|txt))?/(?P<info>.+)?/?$'%AuthentaConfig.getRegTask(), views.TaskCreate.as_view(), name='TaskCreate'))
#urlpatterns.append(url(r'^authenta/task/update/(?P<tasktype>%s).?(?P<extension>(html|json|txt))?/(info=)?(?P<info>.+)?/(error=)?(?P<error>.+)?/?$'%AuthentaConfig.getRegTask(), views.TaskUpdate.as_view(), name='TaskUpdate'))
