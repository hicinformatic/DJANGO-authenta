...
class Method(models.Model):
    ...
    groups = models.ManyToManyField(Group, blank=True, verbose_name=_('Groups associated'))
    permissions = models.ManyToManyField(Permission, blank=True, verbose_name=_('Permissions associated'))
    ...
    vn_ldapcacert = _('Certificat LDAP')
    ht_ldapcacert = _('Uploade here the certificat to check')
    vn_ldapdefine = _('Base DN ex: dc=domain,dc=com')
    ht_ldapdefine = _('Base DN')
    vn_ldapscope = _('Scope')
    ht_ldapscope = _('Choice a scope')
    vn_ldapversion = _('Version')
    ht_ldapversion = _('Choice a version')
    vn_ldapbind = _('Bind DN')
    ht_ldapbind = _('Bind for override user permission, ex: cn=manager,dc=domain,dc=com (Keep null if not used)')
    vn_ldappassword = _('Password')
    ht_ldappassword = _('Password used by the bind')
    vn_ldapuser = _('User DN')
    ht_ldapuser = _('Replace Bind DN by a User DN. <strong>Do not use with Bind DN</strong> | user DN ex : uid={{username}},ou=my-ou,dc=domain,dc=com | Available tags: {{username}}')
    nv_ldapsearch = _('Search DN')
    ht_ldapsearch = _('search DN (ldap filter) ex : (&(uid={{username}})(memberof=cn=my-cn,ou=groups,dc=hub-t,dc=net)) | Available tags: {{username}}')

    ldap_host = models.CharField(vn_ldaphost, blank=True, help_text=ht_ldaphost, max_length=254, null=True)
    ...
    ldap_cacert = models.FileField(vn_ldapcacert, blank=True, help_text=ht_ldapcacert, null=True, upload_to=ldap_certsdir)
    ...
    ldap_version = models.CharField(ht_ldapversion, choices=AuthentaConfig.choices_ldapversion, default='VERSION2', help_text=ht_ldapversion, max_length=8)
